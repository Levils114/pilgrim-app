...
export default function HeaderToProductScreen({ product }: IHeaderToProductScreen){
    ...
    return(
        <Header>
            ...
            <TouchableOpacity onPress={() => handleAddProductInCart(product)}>
                <IoniconsIcons 
                    name={productsInCart.some(productFinded => productFinded.id === product.id) ? 'cart' : 'cart-outline'} 
